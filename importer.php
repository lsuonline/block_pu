...
function block_pu_codeimport() {
    ...
    $content = block_pu_getcccontent($filename);

    // Import the CSV into the DB.
    $success = block_pu_ccimport($content);
 ...
 *
 */
function block_pu_guildimporter() {
    global $CFG;
    ...
    // Load the content based on the filename / location.
    $content = block_pu_getguildcontent($filename);

    // Import the CSV into the DB.
    $success = block_pu_guildimport($content);
 ...
 *
 */
function block_pu_ccimport($content) {
    ...
    foreach ($content as $line) {
        ...
        if (!empty($fields[0])) {
            ...
            $success = block_pu_ccfield2db($fields);
 ...
 *
 */
function block_pu_guildimport($content) {
    ...
    foreach ($content as $line) {
        ...
        if (!empty($fields[0]) && !empty($fields[1])) {
            ...
            // Add the data to the DB.
            $success = block_pu_guildfield2db($fields);
 ...
 *
 */
function block_pu_getcccontent($filename) {
 ...
 *
 */
function block_pu_getguildcontent($filename) {
 ...
 *
 */
function block_pu_ccfield2db($fields) {
    global $DB;
 ...
 *
 * @package   block_pu
 * @param     array $fields course and user
 * @return    int
 *
 */
function block_pu_guildfield2db($fields) {
    global $CFG, $DB;
